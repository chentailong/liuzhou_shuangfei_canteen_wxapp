    ...
    if (!this.data.userAddr && this.data.type==0) {
      ...
      if(this.data.showUserAddr){
        wx.showModal({
          ...
          title:'您的配送地址信息为: ',
          cancelText:'修改',
          content:`${orderList.floorName}${orderList.departmentName}${orderList.bedNum}-${orderList.receiver}${orderList.mobile}`,
          success:(res)=>{
            if(res.confirm){
  ...
  calWeixinPay: function () {
    ...
    var params = {
      ...
      callBack: function (res) {
        ...
        //       } else if (res.cancel) {
        //         wx.redirectTo({
        //           url: '/pages/pay-result/pay-result?sts=0&orderNumbers=' + that.data.orderIds + "&orderType=" + that.data.payType,
        //         })
        //       }
        //     }
        //   })
